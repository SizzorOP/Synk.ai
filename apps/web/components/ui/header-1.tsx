...
export function Header() {
	...
	return (
		<header
			className={cn('fixed top-0 z-50 w-full transition-all duration-300 border-b border-transparent bg-[#030303]/90 backdrop-blur-md', {
				'border-white/[0.08] shadow-2xl': scrolled,
			})}
		>
			<nav className="mx-auto flex h-16 w-full max-w-7xl items-center justify-between px-6">
				<Link href="/" className="flex items-center gap-2 group cursor-pointer">
					<div className="text-xl font-black tracking-tighter text-[#b22206] transition-transform group-hover:scale-105">synk.ai</div>
				</Link>
				<div className="hidden items-center gap-6 md:flex">
					{links.map((link) => (
						<a 
                            key={link.label} 
                            className="font-['Inter'] font-bold tracking-tight uppercase text-xs text-white/60 hover:text-[#b22206] transition-colors duration-150" 
                            href={link.href}
                        >
							{link.label}
						</a>
					...
					<Link href="#roles">
                        <HoverBorderGradient
                            ...
                            className="bg-[#b22206] text-white font-bold text-[10px] uppercase tracking-widest py-2 px-6"
                        >
                            Get Started
                        </HoverBorderGradient>
                    </Link>
			...
			<MobileMenu open={open} className="flex flex-col justify-between gap-4">
					...
					{links.map((link) => (
						<a
							...
							className={cn(
                                buttonVariants({
                                    variant: 'ghost',
                                    className: 'justify-start text-white/70 hover:text-white hover:bg-white/10 h-12 text-sm font-bold uppercase tracking-widest',
                                })
                            )}
							href={link.href}
						>
				...
				<div className="flex flex-col gap-3 py-4">
					<Link href="#roles" className="w-full block" onClick={() => setOpen(false)}>
                        <HoverBorderGradient
                            as="div"
                            containerClassName="w-full rounded-full"
                            className="w-full bg-[#b22206] text-white py-4 font-black uppercase tracking-widest text-center text-sm"
                        >
                            Get Started
                        </HoverBorderGradient>
                    </Link>
	...
	return createPortal(
		...
		>
			<div
				...
				className={cn(
					'data-[slot=open]:animate-in data-[slot=open]:slide-in-from-top-4 duration-300 ease-out',
					'size-full p-6',
					className,
				)}
				{...props}
